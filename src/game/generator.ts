...
import { DEFAULT_LAYOUT, NPC_DATA, WEAPONS, shuffle, pickRandom, pickOne } from "./houseLayouts";
 ...
 */
function createNPCs(npcData: Array<{ name: string; gender: Gender }>, murdererName: string, rooms: Room[]): NPC[] {
...
function createItems(
	murderWeapon: { name: string; description: string; icon: string },
	murderRoom: Room,
	rooms: Room[],
): Item[] {
	const items: Item[] = [];
 ...
 * TODO: This also makes the solution too obvious. We'll want to fix this at some point.
 */
function assignAlibis(npcs: NPC[], rooms: Room[]): void {
	...
	// TODO: Murderer could falsy claim to have been with someone, but that person says they were with their alibi partner. It's less obvious than a "no alibi".
...
export function getItemsInRoom(state: GameState, roomId: string): Item[] {
	return state.items.filter((item) => item.room === roomId && !state.inventory.includes(item.id));
}
 ...
 * Check win condition
 */
export function checkWinCondition(state: GameState, accusedNpcId: string): "win" | "lose" {
